...
import { RankingUser } from "../../components/ranking_user/ranking_user.jsx"

import silver_icon from "../../../assets/specific_page/home/silver_icon.png"
import bronze_icon from "../../../assets/specific_page/home/bronze_icon.png"
import gold_icon from "../../../assets/specific_page/home/gold_icon.png"
...
import ranking_icon from "../../../assets/specific_page/home/ranking_icon.png"
...
export function Home() {
    ...
    const [rankingUsers, setRankingUsers] = useState([])
     const [rankingUsersLowest, setRankingUsersLowest] = useState([])

    useEffect(() => {
        if (rankingStatus) {
            axios.get("http://localhost:5000/api/get_users_ranking")
                .then((res) => {
                    console.log(res)
                    setRankingUsers(res.data.message)

                    setRankingUsersLowest(res.data.message.map((user,i)=>{
                        if(i>3){
                            return <RankingUser name={user.name} i={i} xp={user.xp}/>
                        }
        ...
        <div id="home">
            ...
            {connected == false ? <TokenInvalid token_error={tokenError} /> :
                <div id="home_principal">
                        ...
                        <div id="home_content_principal">
                            ...
                            <div id="home_content_principal_right">
                                <div id="home_content_left_part">
                                    ...
                                    <p id="raking_title">Ranking:</p>
                                    <div id="ranking_page" onClick={() => {
                                        setRankingStatus(true)
                                    }}><img src={ranking_icon} /><p>Posições</p></div>
                    ...
                    {
                        rankingStatus ?
                            <div id="ranking_background" onClick={() => {
                                setRankingStatus(false)
                            }}>
                                <div id="ranking_bar" onClick={(e) => {
                                    e.stopPropagation()
                                }}>
                                    <div id="ranking_bar_top">
                                        <h1>Ranking</h1>
                                        <button onClick={() => {
                                            setRankingStatus(false)
                                        }}>X</button>
                                    </div>
                                    <div id="ranking_bar_middle">

                                        <div className="ranking_bar_middle_inline">
                                            <div className="ranking_bar_middle_profile">
                                                <img src={`data:image/png;base64,${rankingUsers[1]?.image}`} />
                                            </div>
                                            <div id="ranking_classification_silver">
                                                <div><img src={silver_icon} /><h3>{rankingUsers[1]?.name}</h3></div>
                                                <p>XP:{rankingUsers[1]?.xp}</p>
                                            </div>
                                        </div>
                                        <div id="ranking_bar_middle_middle">
                                            <div className="ranking_bar_middle_profile">
                                                <img src={`data:image/png;base64,${rankingUsers[0]?.image}`} />
                                            </div>
                                            <div id="ranking_classification_gold">
                                                <div><img src={gold_icon} /><h3>{rankingUsers[0]?.name}</h3></div>
                                                <p>XP:{rankingUsers[0]?.xp}</p>
                                            </div>
                                        </div>
                                        <div className="ranking_bar_middle_inline">
                                            <div className="ranking_bar_middle_profile">
                                                <img src={`data:image/png;base64,${rankingUsers[2]?.image}`} />
                                            </div>
                                            <div id="ranking_classification_bronze">
                                                <div><img src={bronze_icon} /><h3>{rankingUsers[2]?.name}</h3></div>
                                                <p>XP:{rankingUsers[2]?.xp}</p>
                                            </div>
                                        </div>

                                    </div>
                                    <div id="ranking_bar_bottom">
                                        <div id="ranking_bar_bottom_list_title">
                                            <p>Posição</p>
                                            <p>Nome</p>
                                            <p>XP</p>
                                        </div>
                                        <div id="ranking_bar_bottom_list">
                                            {rankingUsersLowest}
                                        </div>
                                    </div>
                                </div>
